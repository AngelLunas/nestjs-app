...
export class CodaService {
    ...
    public async getIdsPage(page: string): Promise<string[]> {
        ...
        if (docId && tableId) {
            ...
            try {
                const response = await axios.get(`https://coda.io/apis/v1/docs/${docId}/tables/${tableId}/rows`, {
                    headers,
                });
                //Para cada item, obtengo el id y lo agrego a un array
                const ids: string[] = response.data.items.map((item: any) => {
                ...
                console.error(error);
                throw new HttpException('Error al obtener los ids de la página', 500);
            }
        } else {
