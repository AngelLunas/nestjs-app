...
export class TasksService {
    ...
    async handleCron() {
        ...
        const randomDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 19, 11);//new Date(now.getFullYear(), now.getMonth(), now.getDate(), randomHour + 1, randomMinute);
        //this.logger.debug(`Scheduling LocationAvailabilityDtos job at ${randomHour + 1}:${randomMinute}`);

        this.scheduleGetAvailabilityOfPlacesJobRandom('0 11 19 * * *', randomDate);
    }   
}